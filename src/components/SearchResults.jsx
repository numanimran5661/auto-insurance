...
const SearchResults = () => {
    let { stateCode, city } = useParams();
    const { state } = useLocation();
    return (<>
        <div>
            <Box sx={{ boxShadow: '0 .125rem .25rem rgba(0,0,0,.075)', margin: 'auto' }} p={3} maxWidth='500px'>
                <Stack direction='column'>
                    <Typography variant='h4' fontWeight='bold' textAlign='center' pb={3}>{city} Auto Insurance Quote</Typography>
        ...
        <h2>Results: {"auto-insurance"} {"/"} {stateCode} {"/"} {city} {"/"}</h2>
        <div>
            <SearchPlaces
